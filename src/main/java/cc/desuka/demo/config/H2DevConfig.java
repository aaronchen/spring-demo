...
/**
 * H2 database tooling — only active in the {@code dev} profile. Starts the H2 web server on port
 * 8082 and registers the H2 console servlet.
 */
@Configuration
@Profile("dev")
public class H2DevConfig {
    ...
    public CommandLineRunner startH2WebServer() {
        return args -> {
            try {
                ...
                webServer.start();
                System.out.println(
                        "H2 web server started on port 8082 (browse http://127.0.0.1:8082)");
    ...
    public ServletRegistrationBean<?> h2ConsoleServlet() {
        try {
            ...
            jakarta.servlet.http.HttpServlet placeholder =
                    new jakarta.servlet.http.HttpServlet() {
                        ...
                        protected void doGet(
                            ...
                            resp.sendError(
                                    404,
                                    "H2 console servlet not available (no Jakarta H2 servlet)");
                        }
                    };
