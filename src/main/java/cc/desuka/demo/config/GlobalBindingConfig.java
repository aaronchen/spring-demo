...
public class GlobalBindingConfig {
    ...
    // StringTrimmerEditor(true) does two things: trims whitespace, and converts empty strings to
    // null.
    //
    // Affected (WebDataBinder-based binding):
    //   - @ModelAttribute fields (form submissions bound to DTOs, e.g. TaskRequest,
    // AdminUserRequest)
    ...
    //   - @RequestBody (JSON deserialized by Jackson — uses its own ObjectMapper, not
    // WebDataBinder)
    //   - Values read directly from HttpServletRequest (e.g. request.getParameter())
    //
    // Effect: " hello " → "hello", "  " → null, "" → null
    // This means @NotBlank catches blank/empty input naturally — no manual .trim() needed in
    // controllers.
    @InitBinder
