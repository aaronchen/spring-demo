 ...
 *
 * <p>Constructed per-request by {@link AuthDialect} with the currently authenticated {@link User}
 * (or {@code null} for anonymous requests).
 *
 ...
 *
 * <ul>
 *   <li>{@code ${#auth.isOwner(task)}} — true if current user owns the entity
 *   <li>{@code ${#auth.isAdmin()}} — true if current user has ADMIN role
 *   <li>{@code ${#auth.canEdit(task)}} — true if admin or owner (use for UI visibility)
 * </ul>
 *
 * @see AuthDialect
 * @see OwnedEntity
...
public class AuthExpressions {
     ...
     * expression objects must be instances, not static method holders. It delegates to the static
     * overload which holds the actual comparison logic.
     *
    ...
    /**
     * Returns {@code true} if the current user can edit the given entity — either as its owner or
     * as an admin. Use in templates to control edit/delete button visibility.
     *
     * <p>Does NOT cover special cases like unassigned entities — those are business decisions that
     ...
     *
     * <p>Ownership requires both a non-null user and an assigned entity owner, compared via {@link
     * User#equals(Object)} (which checks {@code id} only).
     *
     * <p>This static overload exists so server-side code (e.g. controller guard methods) can reuse
     * the same ownership logic without constructing an {@code AuthExpressions} instance or
     * depending on the Thymeleaf dialect.
     *
    ...
    /**
     * Static overload for server-side admin checks (e.g. {@link
     * cc.desuka.demo.security.OwnershipGuard}).
     */
